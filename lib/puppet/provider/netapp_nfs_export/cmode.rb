...
Puppet::Type.type(:netapp_nfs_export).provide(:cmode, :parent => Puppet::Provider::NetappCmode) do
  @doc = "Manage Netapp NFS export creation, modification and deletion on CMode."
  ...
  def self.instances
    Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: got to self.instances.")
    exports = []
    ...
    rules.each do |rule|
      ...
      Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode.prefetch: Processing rule for export #{name}. \n")
  ...
  def self.prefetch(resources)
    Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: Got to self.prefetch.")
  ...
  def flush
    Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: Got to flush for resource #{@resource[:name]}.")

    # Check required resource state
    Puppet.debug("Property_hash ensure = #{@property_hash[:ensure]}")
    case @property_hash[:ensure]
    when :absent
      Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: Ensure is absent.")
      ...
      Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: export rule #{@resource[:name]} destroyed successfully. \n")
      return true

    when :present
      Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: Ensure is present.")
      ...
      unless @resource[:readwrite].nil?
        ...
        if Array(@resource[:preffered_ad_servers]).first == 'all_hosts'
          ...
          Array(@resource[:preffered_ad_servers]).each do |host|
            hostname_info = NaElement.new("exports-hostname-info")
      ...
      unless @resource[:readonly].nil?
        ...
        Puppet.debug("Got a readonly array. Checking if all_hosts... First record = #{Array(@resource[:readonly]).first} \n")
      ...
      Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: export rule #{@resource[:name]} modified successfully on path #{@resource[:path]}. \n")
      return true
  ...
  def create
    Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: creating Netapp export rule #{@resource[:name]} on path #{@resource[:path]}.")
    ...
    if(loaded_paths.nil?)
      Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: export rule #{@resource[:name]} creation failed. \n")
      raise Puppet::Error, "Puppet::Provider::Netapp_nfs_export.cmode: export rule #{@resource[:name]} creation failed. Verify settings. \n"
      return false
    end

    # Passed above, therefore must of worked.
    Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: export rule #{@resource[:name]} created successfully on path #{@resource[:path]}. \n")
    return true
  end

  def destroy
    Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: destroying Netapp export rule #{@resource[:name]} against path #{@resource[:path]}")
    @property_hash[:ensure] = :absent
  end

  def exists?
    Puppet.debug("Puppet::Provider::Netapp_nfs_export.cmode: checking existance of Netapp export rule #{@resource[:name]}.")
